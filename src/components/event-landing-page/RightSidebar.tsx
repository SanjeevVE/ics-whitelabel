...
const RightSidebar = ({ event, latitude, longitude, formatEventDate }: RightSidebarProps) => {
  const [isSticky, setIsSticky] = useState<boolean>(false);

  // Scroll handler to make sidebar sticky
  useEffect(() => {
    const handleScroll = () => {
      // Start being sticky after scrolling down 200px
    ...
    >
      ...
      <div className="mt-4 p-6 border border-gray-200 rounded-lg shadow-md bg-white">
        ...
        <div className="mb-4 h-64 w-full bg-gray-200 rounded-lg flex items-center justify-center overflow-hidden">
          {latitude && longitude ? (
            <iframe
              src={`https://maps.google.com/maps?q=${event.location}&output=embed&ll=${latitude},${longitude}&z=8&iwloc`}
              width="100%"
              height="100%"
              allowFullScreen
              loading="lazy"
        ...
        <div>
          ...
          {event.orgEmail && (
            <div className="flex items-center mb-2">
              ...
              <a href={`mailto:${event.orgEmail}`} className="hover:text-blue-600">
                {event.orgEmail}
              </a>
            </div>
          )}
          {event.contactNum && (
            <div className="flex items-center mb-2">
              ...
              <a href={`tel:${event.contactNum}`} className="hover:text-blue-600">
                {event.contactNum}
              </a>
            </div>
          )}
        ...
        {/* Support */}
        <div>
          <h3 className="text-lg font-bold mb-2">Registration Platform Support:</h3>
          <div className="flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            <a href="mailto:support@novarace.in" className="hover:text-blue-600">
              support@novarace.in
            </a>
          </div>
        </div>
      ...
      <div className="flex justify-end items-center gap-2 mt-4">
        {/* WhatsApp */}
        <button 
          onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(`Check out ${event.eventName}: ${window.location.href}`)}`)}
        ...
        {/* Twitter/X */}
        <button 
          onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Check out ${event.eventName}`)}&url=${encodeURIComponent(window.location.href)}`)}
